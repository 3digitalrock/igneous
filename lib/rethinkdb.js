...
// All videos is separated so we don't always grab unapproved videos by default
module.exports.getAllVideos = function (options, callback) {
  if(!options.limit){options.limit=20;}
  if(!options.offset){options.offset=0}
  onConnect(function (err, connection) {
    console.log("[INFO][%s][getAll videos]", connection['_id']);
    
    if(!options.order){
      r.db(dbConfig['db']).table('videos').filter(r.row('status').lt(options.status)).skip(options.offset).limit(options.limit).pluck(options.fields).run(connection, function(err, cursor) {
        if(err) {
          console.log("[ERROR][%s][getAll videos] %s:%s\n%s", connection['_id'], err.name, err.msg, err.message);
          callback(null, null);
        }
        else {
          cursor.toArray(function(err, results) {
      ...
      r.db(dbConfig['db']).table('videos').orderBy(options.orderOptions).filter(r.row('status').lt(options.status)).skip(options.offset).limit(options.limit).pluck(options.fields).run(connection, function(err, cursor) {
        if(err) {
          console.log("[ERROR][%s][getAll videos] %s:%s\n%s", connection['_id'], err.name, err.msg, err.message);
          callback(null, null);
        }
...
module.exports.getRelated = function (options, related, callback) {
  if(!options.limit){options.limit=4;}
  onConnect(function (err, connection) {
    console.log("[INFO][%s][get related %s from %s %s]", connection['_id'], options.model, options.id);
    
    if(related=="studio"){
      r.db(dbConfig['db']).table(options.model).getAll(options.studio, {index: options.indexKey}).filter(r.not(r.row('uid').eq(options.id)).and(r.row('status').eq('1'))).pluck(options.fields).limit(options.limit).run(connection, function(err, cursor) {
        if(err) {
      ...
      r.db(dbConfig['db']).table(options.model).filter(r.row('channels').contains(options.channels).and(r.not(r.row('uid').eq(options.id))).and(r.row('status').eq('1'))).pluck(options.fields).limit(options.limit).run(connection, function(err, cursor) {
          if(err) {
            console.log("[ERROR][%s][get related %s] %s:%s\n%s", connection['_id'], options.model, err.name, err.msg, err.message);
            callback(null, null);
          }
  ...
  onConnect(function (err, connection) {
    r.db(dbConfig['db']).table(model).insert(body).run(connection, function(err, result) {
      if(err) {
        ...
        console.log("[INFO][%s][created %s]", connection['_id'], model);
