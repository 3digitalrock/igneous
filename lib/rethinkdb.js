...
module.exports.getSome = function (options, callback) {
  ...
  onConnect(function (err, connection) {
    ...
    if(!options.order){
      r.db(dbConfig['db']).table(options.model).filter(eval(options.filter)).pluck(options.fields).limit(options.limit).run(connection, function(err, cursor) {
        if(err) {
          console.log("[ERROR][%s][get %s] %s:%s\n%s", connection['_id'], options.model, err.name, err.msg, err.message);
          callback(null, null);
        }
      ...
      r.db(dbConfig['db']).table(options.model).orderBy({index: r.desc(options.order)}).filter(eval(options.filter)).pluck(options.fields).limit(options.limit).run(connection, function(err, cursor) {
        if(err) {
          console.log("[ERROR][%s][get %s] %s:%s\n%s", connection['_id'], options.model, err.name, err.msg, err.message);
          callback(null, null);
        }
