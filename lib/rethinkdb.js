...
module.exports.getSome = function (model, comparison, id, fields, callback) {
  onConnect(function (err, connection) {
    console.log("[INFO][%s][get %s from %s %s]", connection['_id'], model, id, comparison);
    
    r.db(dbConfig['db']).table(model).filter(r.row(comparison).contains(id)).pluck(fields).run(connection, function(err, cursor) {
      if(err) {
        console.log("[ERROR][%s][get %s] %s:%s\n%s", connection['_id'], model, err.name, err.msg, err.message);
        callback(null, null);
      }
